<!--- Update Existing Post AJAX Endpoint --->
...
<cftry>
    ...
    <cfif not structKeyExists(postData, "id") or len(trim(postData.id)) eq 0>
    ...
    <cfset uniqueSlug = ensureUniqueSlug(baseSlug, postType, trim(postData.id))>
    ...
    <cfelseif postRecord.status eq "published">
        ...
        <cfset existingPost = queryExecute("SELECT status, published_at FROM posts WHERE id = ?", [postRecord.id], {datasource: "blog"})>
        <cfif existingPost.recordCount gt 0 and existingPost.status[1] neq "published">
            <cfset postRecord.published_at = now()>
        </cfif>
    </cfif>
    
    <!--- Update post in database --->
    <cfset result = updatePost(postRecord)>
    
    <cfif result.success>
        <!--- Handle tags if provided --->
        <cfif structKeyExists(postData, "tags") and isArray(postData.tags)>
            <cfset tagResult = updatePostTags(postRecord.id, postData.tags)>
    ...
    <cfcatch any>
        <cfset response.message = "Error updating post: " & cfcatch.message>
        <cfset response.MESSAGE = response.message>
    </cfcatch>
</cftry>
...
<cffunction name="ensureUniqueSlug" access="private" returntype="string">
    ...
    <cfloop condition="true">
        <cfquery name="checkQuery" datasource="blog">
...
<!--- Helper function to update tags for post --->
<cffunction name="updatePostTags" access="private" returntype="struct">
    ...
    <cftry>
        ...
        <cfcatch any>
            ...
            <cfset result.message = "Error updating tags: " & cfcatch.message>
        </cfcatch>
    </cftry>
