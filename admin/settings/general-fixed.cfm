...
<!--- Get current settings using correct column name --->
<cfquery name="getSettings" datasource="#request.dsn#">
    SELECT `key` as setting_key, `value` as setting_value, type 
    FROM settings
    WHERE `key` IN (
...
<cfloop query="getSettings">
    <cfset settings[getSettings.setting_key] = getSettings.setting_value>
</cfloop>
...
<p>General Settings page is loading correctly with the fixed column names.</p>
